...
const semanticPhrases = new RegExp(protectedTerms.source + '|光之十一|光之境|心灵成长|心靈成長|灵修洞见|靈修洞見|所学所感|所學所感|行走|学习|學習|亲身经历|親身經歷|' + phrases.source, 'g')
...
export function aboutView() {
  return `<section class="about-page">
    ...
    <nav class="about-toc" aria-label="${t('about.toc')}" id="about-contents">
      ${[['journey', 'about.journeyTitle'], ['creation', 'about.creationsTitle'], ['ventures', 'about.venturesTitle'], ['practice', 'about.professionalTitle'], ['research', 'about.researchTitle']].map(([id, key]) => `<a href="#${id}">${t(key)}</a>`).join('')}
    </nav>
