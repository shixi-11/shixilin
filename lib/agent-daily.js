...
export function validDailyPath(value) {
  return typeof value === 'string' && value.length < 300 &&
    ...
    /^(?:(?:(?:en|zh-Hant|ja|ko|es|fr|de|ar)\/)?(?:(?:latest\/?|20\d{2}\/\d{2}\/\d{2}\/?|archive\.json|feed\.xml))?|(?:en|zh-Hant|ja|ko|es|fr|de|ar)\/?|assets\/[a-zA-Z0-9_./-]+|sitemap\.xml|robots\.txt|404\.html|feed\.xml)$/.test(value);
}
