...
export function paperFooter(showSupport = false) {
  return `<footer class="paper-footer">
    ...
    <p>${t('home.subtitle')}</p>
    <div class="paper-socials">${socialProfiles.map(profile => `${profile.id === 'youtube' ? '<span aria-hidden="true"></span>' : ''}<a href="${profile.href}" target="_blank" rel="noopener" aria-label="${profile.name} · ${profile.handle}" title="${profile.name} · ${profile.handle}">${profile.icon}</a>`).join('')}</div>
