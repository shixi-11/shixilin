...
export function poetryView(source) {
  ...
  return `<section class="poetry-book${book.title.length > 24 ? ' poetry-long-title' : ''}" lang="${locale.lang}" dir="${locale.dir || 'ltr'}">
    ...
    <header class="poetry-frontispiece">
      ${book.cover ? `<img class="poetry-book-cover" src="${book.cover}" width="${book.coverWidth || 900}" height="${book.coverHeight || 1200}" alt="${escape(book.title)} · ${ui.cover}" decoding="async" />` : `<div class="poetry-title-leaf" aria-hidden="true" lang="zh-Hans"><span>一念凡生</span><small>光之十一</small></div>`}
